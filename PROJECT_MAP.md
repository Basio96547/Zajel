...
| `build/` · `node_modules/` · `.gradle/` · `.kotlin/` | ~600 م.ب مولَّدة بالكامل، تُبنى من جديد. |
...
1. **لا يوجد مستودع Git.** 20,592 سطراً بلا تاريخ ولا نسخة احتياطية. أخطر بند هنا. وعند التهيئة يجب أن يسبق `.gitignore` أول commit ويستبعد `build/` و`node_modules/` و`.gradle/` و`.kotlin/` و**كلا** موضعَي `.wrangler/` و`local.properties`.
2. **~600 م.ب مخرجات بناء** قابلة للحذف بأمان (تُبنى من جديد، بكلفة إعادة بناء).
