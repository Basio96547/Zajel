...
| `relay/node_modules/` | **باقٍ — 190.2 م.ب.** أُبقي عمداً: استعادته تحتاج `npm install` أي **اتصال شبكة**، بخلاف مخرجات Gradle. احذفه متى شئت بشرط توفّر الشبكة لاحقاً. |
...
2. ~~نحو 600 م.ب مخرجات بناء.~~ **حُلَّ جزئياً 2026-07-31:** حُذف 428.8 م.ب من مخرجات Gradle (620.7 → 191.9 م.ب)، وبقي `relay/node_modules` (190.2 م.ب) لأن استعادته تحتاج شبكة. تُحقّق أن البناء لم ينكسر بـ`gradlew projects` → الوحدات الأربع تُحلّ سليمة. أول بناء لاحق سيكون أبطأ، وهي الكلفة المعروفة.
