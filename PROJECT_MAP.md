...
| `:app` | تطبيق أندرويد | التطبيق الرئيسي، يظهر باسم **«زاجل»** ويفتح على حاسبة عاملة تحرس الدخول. |
...
### 5.5 المسار الثالث، الاختياري — دليل اليوزرنيمات ⚠️ مكتوب، **غير منشور بعد**

خلافاً لـ5.2 أعلاه، لا يوجد هنا تحقّق نشر مثل `GET .../` → 405 — هذا القسم يوثّق كوداً مكتوباً ومُختبَراً محلياً فقط، لا خدمة حيّة. لا تُعامَل كأنها منشورة حتى يُحدَّث هذا السطر بعد تشغيل `wrangler deploy` فعلياً والتحقّق كما جرى مع 5.2.

- الكود: `directory/src/index.ts` — خدمة Cloudflare Worker **منفصلة تماماً** عن `relay/` (D1 لا Durable Objects، `observability` **مُفعَّلة** لا مُطفأة). المسارات: `POST /claim`، `GET /u/:username`، `POST /introductions/deposit`، `POST /introductions/fetch`.
- التفصيل الكامل — الفكرة، نموذج الثقة، بالضبط ماذا يرى المُشغِّل — في قسم «🔎 البحث باليوزرنيم» في [`README.md`](README.md)، لا يُكرَّر هنا.
- **حالة الاختبار:** `core/.../net/DirectoryProtocolTest.kt` مُشغَّل فعلاً عبر `gradlew :desktop:test` (10 اختبارات، بايتات مُقارَنة مع `directory/src/crypto.ts` نفسه). فحص شبكي محلي (سكربت Node يدوي ضد `wrangler dev`، لا `gradlew`) أكّد كل مسار (حجز/بحث/إيداع/تحصيل/تصادم اسم/توقيع خاطئ/قراءة مُتلِفة/تحديد معدّل) — لكن هذا ليس نشراً حقيقياً ولا اختباراً على جهاز أندرويد فعلي. `app/androidTest/.../IntroductionRoundtripTest.kt` مكتوب على غرار `RelayRoundtripTest.kt` تماماً، `assumeTrue`-مُتجاوَز تلقائياً ما دام `directoryUrl` فارغاً، وينتظر نشراً فعلياً ليُشغَّل ويُسجَّل رقمه في §8 كأي اختبار آخر هنا.
- `directoryUrl` في `gradle.properties` **فارغ الآن عمداً** — لا مسار مبنيّ في التطبيق حتى تُنشَر الخدمة ويُضبَط هذا السطر يدوياً، تماماً كإفراغ `relayUrl` في 5.4 لكن بالاتجاه المعاكس (الإعداد الافتراضي هنا هو الأشدّ خصوصية، لا العكس).
